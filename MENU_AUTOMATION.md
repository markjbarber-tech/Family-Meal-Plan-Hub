...
- `favourites` — recipes the family has saved from past weeks via the Hub's ❤️ Favourites page. Each has `id`, `name`, `authored_by`, `recipe` `{ingredients, method}`, `saved_at`, and `requested_for_next_week` (a boolean the family sets from that page to request it be guaranteed a spot in the next proposed week)
...
6. Regenerate `index.html`: replace its `WEEK_DATA` constant with the new `current_week`, `PRINCIPLES_TEXT` with `household.key_principles`, and `FAVOURITES` with `household.favourites` (only actually changes if step 5 cleared a `requested_for_next_week` flag, but keep it in sync every time regardless), keeping everything else in the file exactly as-is (feedback box, Log buttons, recipe pages, shopping-list copy button, `FEEDBACK_ENDPOINT` unchanged).
