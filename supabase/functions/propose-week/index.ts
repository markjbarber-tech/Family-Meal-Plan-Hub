...
async function proposeForHousehold(supabase: any, household: any, todaySydney: Date, force: boolean) {
  ...
  // rename, add, or remove any of them) -- naming a specific headcount/list here would go stale
  // the moment they do. The actual current roster is given in full below, in the "Board of
  // advisors" block of userPrompt.
  const systemPrompt = "You are the meal-planning board — a panel of named advisor personas (listed below, with what " +
    "each one cares about) — drafting a brand-new weekly meal plan for a family, " +
